...
model XRA_Baseline_AE_127
  ...
  hil_flexlab_model.BaseClasses.Controls.RTU rTU(
    ...
    annotation (Placement(transformation(extent={{-30,-224},{78,-10}})));
...
equation
  ...
  connect(rTU.yFan, rtu.uFan) annotation (Line(
        points={{83.4,-20.7},{167.15,-20.7},{167.15,14.4},{243.2,14.4}},
        color={0,0,127}));
  connect(rTU.yHea, rtu.uHea) annotation (Line(
        points={{83.4,-59.6091},{179.15,-59.6091},{179.15,-20.4},{243.2,-20.4}},
        color={0,0,127}));
  connect(rTU.yCoo, rtu.uCoo) annotation (Line(
        points={{83.4,-88.7909},{164.15,-88.7909},{164.15,-55.2},{243.2,-55.2}},
        color={0,0,127}));
  connect(rTU.yOutAirFra, rtu.uEco) annotation (Line(
        points={{83.4,-127.7},{166.15,-127.7},{166.15,-90},{243.2,-90}},
        color={0,0,127}));
  connect(rtu.TMixAir, rTU.TMix) annotation (Line(
        points={{432.4,-159.6},{488,-159.6},{488,-378},{224,-378},{224,-195.791},
          {-40.8,-195.791}},
                      color={0,0,127}));
  connect(rTU.TSup, rtu.TSup) annotation (Line(
        points={{-40.8,-224.973},{498.6,-224.973},{498.6,-171.2},{432.4,-171.2}},
        color={0,0,127}));
  connect(rTU.TRet, rtu.phiRetAir) annotation (Line(
        points={{-40.8,-166.609},{-82,-166.609},{-82,-168},{-124,-168},{-124,
  ...
  connect(weaBus.TDryBul, rTU.TEco) annotation (Line(
      points={{-184,290},{-106,290},{-106,-136.455},{-40.8,-136.455}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
  ...
  connect(flexlabXRA.TRooAir, rTU.TRoo) annotation (Line(points={{1364.57,
          -29.8733},{1396,-29.8733},{1396,366},{-90,366},{-90,-108.245},{-40.8,
          -108.245}},
        color={0,0,127}));
  ...
  connect(TSetRooHea.y[1], rTU.TSetRooHea) annotation (Line(points={{-191,-104},
          {-117.5,-104},{-117.5,-49.8818},{-40.8,-49.8818}}, color={0,0,127}));
  connect(TSetRooCoo.y[1], rTU.TSetRooCoo) annotation (Line(points={{-191,-134},
          {-114,-134},{-114,-79.0636},{-40.8,-79.0636}}, color={0,0,127}));
  connect(flexlabXRA.portsCell[2], rtu.returnAir) annotation (Line(points={{
          996.571,-43.5933},{702.572,-43.5933},{702.572,-90},{428,-90}}, color=
          {0,127,255}));
  connect(flexlabXRA.portsCell[1], rtu.supplyAir) annotation (Line(points={{
          973.714,-43.5933},{705.572,-43.5933},{705.572,-43.6},{428,-43.6}},
        color={0,127,255}));
  connect(flexlabXRA.occ, rTU.occSta) annotation (Line(points={{1385.14,100.467},
          {665.57,100.467},{665.57,-20.7},{-40.8,-20.7}}, color={255,0,255}));
  annotation (
