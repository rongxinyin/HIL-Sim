...
partial model PartialFloor_withPlenum
  ...
  parameter Modelica.Units.SI.Volume VRooPle "Room volume plenum";
  ...
  parameter Modelica.Units.SI.Area AFloPle "Floor area plenum";
  ...
  Buildings.Examples.VAVReheat.BaseClasses.RoomLeakage leaSou(
    ...
    annotation (Placement(transformation(extent={{-58,352},{-22,392}})));
  ...
  Modelica.Thermal.HeatTransfer.Sensors.TemperatureSensor temAirple
    "Air temperature sensor"
    annotation (Placement(transformation(extent={{288,312},{308,332}})));
  Buildings.Airflow.Multizone.DoorOpen opePleCor(redeclare package Medium =
        Medium, wOpe=10) "Opening between perimeter3 and core"
    annotation (Placement(transformation(extent={{180,30},{200,50}})));
  Modelica.Fluid.Vessels.BaseClasses.VesselFluidPorts_b portsPle[2](redeclare
      package Medium = Medium) "Fluid inlets and outlets" annotation (Placement(
        transformation(extent={{258,24},{298,40}}), iconTransformation(extent={
            {202,48},{242,64}})));
  Buildings.Examples.VAVReheat.BaseClasses.RoomLeakage leaPle(
    ...
    final use_windPressure=use_windPressure)
    "Model for air infiltration through the envelope"
    annotation (Placement(transformation(extent={{-58,406},{-22,446}})));
equation
  connect(weaBus, leaSou.weaBus) annotation (Line(
      points={{210,200},{-80,200},{-80,372},{-58,372}},
      color={255,204,51},
      thickness=0.5,
      smooth=Smooth.None));
  ...
  connect(leaPle.weaBus, weaBus) annotation (Line(
      points={{-58,426},{-80,426},{-80,200},{210,200}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%second",
      index=1,
