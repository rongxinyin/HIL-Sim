...
partial model partialAirUnit "Partial model for an air supply unit"
  ...
  hil_flexlab_model.BaseClasses.HeatRecoveryIEC ERV(
    redeclare package Medium1 = MediumA,
    redeclare package Medium2 = MediumA,
    m1_flow_nominal=HRSizeMinOAMult*mAir_flow_minOA,
    m2_flow_nominal=HRSizeMinOAMult*mAir_flow_minOA)
    "Energy recovery with indirect evaporative cooling"
    annotation (Placement(transformation(extent={{-120,-20},{-100,0}})));
  Modelica.Blocks.Interfaces.BooleanInput enaEneRec "Activate energy recovery"
    annotation (Placement(transformation(extent={{-240,-120},{-200,-80}})));
  Modelica.Blocks.Interfaces.BooleanInput enaEvaCoo
    "Activate evaporative cooling for energy recovery"
  ...
  Modelica.Blocks.Logical.Switch swiHeaRec
    "Switch for limiting flow through IECHR"
    annotation (Placement(transformation(extent={{-100,80},{-80,100}})));
  Modelica.Blocks.Sources.RealExpression limHeaRec(y=HRSizeMinOAMult*
        mAir_flow_minOA/Buildings.Utilities.Math.Functions.regNonZeroPower(
        gaiFan.y, 1))                     "Upper limit of IECHR airflow"
  ...
  Modelica.Blocks.Sources.RealExpression zero "Zero signal"
  ...
  Buildings.Fluid.Sensors.MassFlowRate senMasFloOut(redeclare package Medium =
        ...
        rotation=90,
        origin={-94,18})));
  Modelica.Blocks.Interfaces.RealOutput TSupIECHR(final unit="K", displayUnit=
        "degC") "IECHR supply air temperature" annotation (Placement(
        transformation(extent={{200,-150},{220,-130}}), iconTransformation(
          extent={{200,-150},{220,-130}})));
...
equation
  ...
  connect(ERV.evaCooSta,enaEvaCoo)  annotation (Line(points={{-122,-4},{-146,-4},
          {-146,-160},{-220,-160}},color={255,0,255}));
  connect(ERV.eneRecSta,enaEneRec)  annotation (Line(points={{-122,0},{-148,0},
          {-148,-100},{-220,-100}},color={255,0,255}));
  connect(ERV.port_a2, out.ports[1]) annotation (Line(points={{-120,-8},{-140,
          -8},{-140,-10},{-160,-10}},
                                color={0,127,255}));
  connect(ERV.port_b1, out.ports[2]) annotation (Line(points={{-120,-16},{-140,
          -16},{-140,-14},{-160,-14}},
                              color={0,127,255}));
  ...
  connect(ERV.port_a1, ideEco.port_3)
    annotation (Line(points={{-100,-16},{-80,-16},{-80,36}},
                                                         color={0,127,255}));
  ...
  connect(swiHeaRec.y, ideEco.y)
    annotation (Line(points={{-79,90},{-80,90},{-80,57}}, color={0,0,127}));
  connect(uEco, swiHeaRec.u3) annotation (Line(points={{-220,-40},{-152,-40},{-152,
          82},{-102,82}}, color={0,0,127}));
  connect(swiHeaRec.u2, enaEneRec) annotation (Line(points={{-102,90},{-148,90},
          {-148,-100},{-220,-100}}, color={255,0,255}));
  connect(limHeaRec.y, variableLimiter.limit1)
    annotation (Line(points={{-139,118},{-132,118}}, color={0,0,127}));
  connect(uEco, variableLimiter.u) annotation (Line(points={{-220,-40},{-152,-40},
          {-152,110},{-132,110}}, color={0,0,127}));
  connect(zero.y, variableLimiter.limit2) annotation (Line(points={{-139,100},{-135.5,
          100},{-135.5,102},{-132,102}}, color={0,0,127}));
  connect(variableLimiter.y, swiHeaRec.u1) annotation (Line(points={{-109,110},{
          -102,110},{-102,98}}, color={0,0,127}));
  ...
  connect(senMasFloOut.port_a, ERV.port_b2) annotation (Line(points={{-94,8},{
          -94,-8},{-99.8,-8}}, color={0,127,255}));
  ...
  connect(ERV.TSup, TSupIECHR) annotation (Line(points={{-99,-4},{-96,-4},{-96,
