...
partial model PartialHVAC
  ...
  constant Integer numZon(min=2)=3 "Total number of served VAV boxes";
  ...
  Modelica.Fluid.Interfaces.FluidPort_a port_supAir[numZon](redeclare package
      Medium = MediumA) "Supply air to thermal zones" annotation (Placement(
        transformation(extent={{1410,150},{1430,170}}), iconTransformation(
          extent={{432,190},{452,210}})));
  Modelica.Fluid.Interfaces.FluidPort_a port_retAir[numZon](redeclare package
      Medium = MediumA) "Return air from thermal zones" annotation (Placement(
        transformation(extent={{1410,110},{1430,130}}), iconTransformation(
          extent={{432,20},{452,40}})));
